...
const Navigation = () => {
  ...
  const data = useSelector((state) => state.covidData.data);
  const isFetched = useSelector((state) => state.covidData.fetched);
  const error = useSelector((state) => state.covidData.error);
  ...
  const filter = (e) => {
    switch (e.target.id) {
      case 'northam':
        setCategories([...data.filter((c) => c.continent === 'North America')]);
        break;
      case 'southam':
        setCategories([...data.filter((c) => c.continent === 'South America')]);
        break;
      case 'europe':
        setCategories([...data.filter((c) => c.continent === 'Europe')]);
        break;
      case 'africa':
        setCategories([...data.filter((c) => c.continent === 'Africa')]);
        break;
      case 'oceania':
        setCategories([...data.filter((c) => c.continent === 'Oceania')]);
        break;
      case 'asia':
        setCategories([...data.filter((c) => c.continent === 'Asia')]);
        break;
      case 'antarctica':
        setCategories([...data.filter((c) => c.continent === 'Antarctica')]);
        break;
      default:
        setCategories(data);
    }
      ...
      return (
        <div className="main">
          ...
          <div className="error-container">
            ...
            <h3>The covid API is sleeping.</h3>
